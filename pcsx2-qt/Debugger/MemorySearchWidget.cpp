...
void searchWorker(DebugInterface* cpu, std::vector<SearchResult>& searchResults, SearchType searchType, SearchComparison searchComparison, u32 start, u32 end, T searchValue)
{
	...
	{
		auto dontMatch = std::remove_if(searchResults.begin(), searchResults.end(), [cpu, searchResults, searchType, searchComparison, searchValue](SearchResult& searchResult) {
			const u32 addr = searchResult.getAddress();
			if (!cpu->isValidAddress(addr))
				return true;
			T readValue = readValueAtAddress<T>(cpu, addr);
			if(handleSearchComparison(searchComparison, addr, &searchResult, searchValue, readValue))
			{
		...
		searchResults.erase(dontMatch, searchResults.end());
	}
}
...
static void searchWorkerByteArray(DebugInterface* cpu, SearchType searchType, SearchComparison searchComparison, std::vector<SearchResult> searchResults, u32 start, u32 end, QByteArray searchValue)
{
	...
	{
		auto dontMatch = std::remove_if(searchResults.begin(), searchResults.end(), [searchComparison, searchType, searchValue, cpu](SearchResult& searchResult) {
			const u32 addr = searchResult.getAddress();
			if (!cpu->isValidAddress(addr))
				return true;
			bool isMatch = handleArraySearchComparison(cpu, searchComparison, addr, &searchResult, searchValue);
			if (isMatch)
			{
...
void MemorySearchWidget::onSearchButtonClicked()
{
	...
	QFuture<std::vector<SearchResult>> workerFuture = QtConcurrent::run(startWorker, m_cpu, searchType, searchComparison, m_searchResults, searchStart, searchEnd, searchValue, searchHex ? 16 : 10);
