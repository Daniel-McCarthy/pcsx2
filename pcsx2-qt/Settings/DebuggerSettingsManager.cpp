...
QJsonObject DebuggerSettingsManager::loadGameSettingsJSON() {
	...
	{
		Console.WriteLn(fmt::format( "Debugger Settings Manager: No Debugger Settings file found for game at: '{}'", path));
		return QJsonObject();
	}
	...
	{
		Console.WriteLn(fmt::format("Debugger Settings Manager: Failed to load contents of settings file for file at: '{}'", path));
		return QJsonObject();
	}
...
void DebuggerSettingsManager::writeJSONToPath(std::string path, QJsonDocument jsonDocument)
{
	...
	{
		Console.WriteLn(fmt::format("Debugger Settings Manager: Failed to write Debugger Settings file to path: '{}'", path));
		return;
	}
...
void DebuggerSettingsManager::loadGameSettings(BreakpointModel* bpModel)
{
	...
	{
		Console.WriteLn(fmt::format("Debugger Settings Manager: Failed to read Breakpoints array from settings file: '{}'", path));
		return;
	}
	...
	for (u32 row = 0; row < breakpointsArray.size(); row++)
	{
		...
		{
			QString headerColKey = bpModel->headerData(col, Qt::Horizontal, Qt::DisplayRole).toString();
...
void DebuggerSettingsManager::loadGameSettings(SavedAddressesModel* savedAddressesModel)
{
	...
	{
		Console.WriteLn(fmt::format("Debugger Settings Manager: Failed to read Saved Addresses array from settings file: '{}'", path));
		return;
	}
	...
	for (u32 row = 0; row < breakpointsArray.size(); row++)
	{
		...
		for (auto iter = rowObject.begin(); iter != rowObject.end(); iter++, col++)
		{
			QString headerColKey = savedAddressesModel->headerData(col, Qt::Horizontal, Qt::DisplayRole).toString();
			fields << rowObject.value(headerColKey).toString();
		}
...
void DebuggerSettingsManager::saveGameSettings(QAbstractTableModel* abstractTableModel, QString settingsKey, u32 role)
{
	...
	{
		keys << abstractTableModel->headerData(col, Qt::Horizontal, Qt::DisplayRole).toString();
	}
